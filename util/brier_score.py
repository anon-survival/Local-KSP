...
# def integrated_brier_score(train_tte, train_event, test_tte, test_event, cdf_test, time):
#     G = KaplanMeierFitter()
#     G.fit(train_tte.cpu().numpy(), 1-train_event.cpu().numpy())

#     surv_test = 1 - cdf_test

#     bs = torch.zeros(len(time), device=DEVICE)

#     ipcw1 = torch.tensor(G.survival_function_at_times((test_tte-1e-8).cpu().numpy()).values, device=DEVICE).view(-1, 1)
#     ipcw1[ipcw1 == 0] = min(ipcw1[ipcw1 != 0])
#     ipcw2 = torch.tensor(G.survival_function_at_times(time.cpu().numpy()).values, device=DEVICE).view(-1)
#     ipcw2[ipcw2 == 0] = min(ipcw2[ipcw2 != 0])

#     uncensored_indicator = (test_tte.unsqueeze(1) <= time) & (test_event.unsqueeze(1) == 1)
#     censored_indicator = (test_tte.unsqueeze(1) > time)

#     uncensored_bs = (uncensored_indicator * ((0 - surv_test) ** 2) / ipcw1).sum(dim=0) / test_tte.shape[0]
#     censored_bs = (censored_indicator * ((1 - surv_test) ** 2) / ipcw2).sum(dim=0) / test_tte.shape[0]

#     bs = uncensored_bs + censored_bs
#     IBS = torch.trapezoid(bs, time) / max(time)
...
def integrated_brier_score(train_tte, train_event, test_tte, test_event, cdf_test, time, batch_size=5000):
    ...
    for i in range(0, N, batch_size):
        ...
        uncensored_bs = (uncensored_indicator * ((1 - cdf_b) ** 2) / ipcw1).sum(dim=0) / N
        censored_bs = (censored_indicator * ((0 - cdf_b) ** 2) / ipcw2).sum(dim=0) / N
