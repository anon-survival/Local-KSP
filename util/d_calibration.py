...
def d_calibration(points, is_dead, args, phase, nbins=20, differentiable=False, gamma=1.0, device='cpu'):
    ...
    # bin_width = torch.tensor([0.05, 0.10, 0.20, 0.40, 0.50, 0.60, 0.80, 0.90, 0.95], device=device)  # 총 10개 bin
    # nbins = len(bin_width)+1

    # bin_a = torch.cat([torch.tensor([0.0], device=device), bin_width]).view(1, -1)
    # bin_b = torch.cat([bin_width, torch.tensor([1.0], device=device)]).view(1, -1)

    # # noise 옵션 적용 (선택적)
    # noise = 1e-6 / nbins * torch.rand(size=(1, nbins), device=device)
    # if not differentiable:
    #     noise = noise * 0
    # cum_noise = torch.cumsum(noise, dim=1)
    # bin_b += cum_noise
