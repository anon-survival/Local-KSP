...
class SyntheticNN(nn.Module):
    def __init__(self, data_dir, D_in, model_dist, dataset, num_cat_bins, hiddensize, dropout_rate, **kwargs):
        ...
        assert model_dist in ['lognormal', 'cox', 'cat', 'psr']
        ...
        if model_dist == 'lognormal':
            ...
            self.model = ProperScoringRule(D_in, num_cat_bins, hidden_sizes, dropout_rate, batchnorm=True)
...
class ProperScoringRule(nn.Module):
    def __init__(self, D_in, D_out, hidden_sizes, dropout_rate, batchnorm=False):
        super(ProperScoringRule, self).__init__()
        self.psr_model = DeepNet(D_in, D_out, hidden_sizes, dropout_rate, batchnorm)

    def forward(self, src):
        pred = self.psr_model(src)
