...
def compute_decensor_times(test_set, train_set):
    t_train, e_train, src_train = train_set
    n_train = len(t_train)
    km_train = KaplanMeierArea(t_train, e_train)

    t_test, e_test, src_test = test_set
    n_test = len(t_test)

    feature_df = src_test
    t = np.repeat(t_test, 1000)
    ...
    for i in range(n_test):
        ...
        if e_test[i] != 1:
            ...
            # surrogate_t = np.random.choice(np.append(uniq_times, last_time), size=1000, p=cond_pdf)
            ...
            t[i * 1000:(i + 1) * 1000] = torch.from_numpy(surrogate_t).float()
