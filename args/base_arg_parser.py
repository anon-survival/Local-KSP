...
class BaseArgParser(object):
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--model', type=str, choices=('SyntheticNN', 'SurvMNISTNN', 'MTLRNN', 'MIMICNN', 'AFTNN'), default='SyntheticNN', help='Model name.')
        self.parser.add_argument('--model_dist', type=str, choices=('lognormal', 'mtlr', 'cox', 'weibull', 'cat', 'psr'), default='lognormal', help='Model name.')
        ...
        self.parser.add_argument('--dataset', type=str, default='synthetic', choices=('mimic', 'synthetic', 'glioma',
                                                                                      ...
                                                                                      'liver', 'stomach', 'bladder'), help='Dataset to use. Gets mapped to dataset class name.')
        ...
        self.parser.add_argument('--synthetic_dist', type=str, default='lognormal', choices=['lognormal', 'weibull', 'mnist'])
        ...
        self.parser.add_argument('--n_quantile', type=float, default=0.5)
        self.parser.add_argument('--use_kernel', type=util.str_to_bool, default=True)
        self.parser.add_argument('--rank', type=str, default='softplus', choices=['softplus', 'exponential'])
        self.parser.add_argument('--kernel', type=str, default='laplacian', choices=['gaussian', 'laplacian'])
        self.parser.add_argument('--node', type=int, default=16)
        self.parser.add_argument('--sample', type=util.str_to_bool, default=True)
        self.parser.add_argument('--B', type=int, default=2000)
    ...
    def parse_args(self):
        ...
        try:
            if args.dataset == 'synthetic':
                ...
                if 'scal' in args.name:
                    if args.model_dist == 'mtlr':
                        save_dir = os.path.join(args.save_dir, args.model_dist + '_' + args.name + '_' + str(args.alpha) + str(args.beta) + '_' + str(args.num_s) + '_ds_' + args.dataset + '_lam' + str(args.lam) + '_bs' + str(args.batch_size) + '_lr' + str(args.lr) + '_optim' + str(args.optimizer) + '_epoch' + str(args.num_epochs) + '_censor' + str(args.censor) + '_seed' + str(args.seed))
                    else:
                    ...
                    if args.model_dist == 'mtlr':
                        save_dir = os.path.join(args.save_dir, args.model_dist + '_' + args.name + '_' + str(args.num_xcal_bins) + '_ds_' + args.dataset + '_lam' + str(args.lam) + '_bs' + str(args.batch_size) + '_lr' + str(args.lr) + '_optim' + str(args.optimizer) + '_epoch' + str(args.num_epochs) + '_censor' + str(args.censor) + '_seed' + str(args.seed))
                    else:
                ...
                if 'scal' in args.name:
                    if args.model_dist in ['mtlr', 'weibull']:
                    ...
                    if args.model_dist == 'mtlr':
                        save_dir = os.path.join(args.save_dir, args.model_dist + '_' + args.name + '_' + str(args.num_cat_bins) + '_ds_' + args.dataset + '_lam' + str(args.lam) + '_bs' + str(args.batch_size) + '_lr' + str(args.lr) + '_optim' + str(args.optimizer) + '_epoch' + str(args.num_epochs) + '_seed' + str(args.seed))
                    elif args.model_dist == 'cat':
                        save_dir = os.path.join(args.save_dir, args.model_dist + '_' + args.name + '_' + str(args.num_cat_bins) + '_ds_' + args.dataset + '_lam' + str(args.lam) + '_dr' + str(args.dropout_rate) + '_bs' + str(args.batch_size) + '_lr' + str(args.lr) + '_optim' + str(args.optimizer) + '_epoch' + str(args.num_epochs) + '_seed' + str(args.seed))
                    elif args.model_dist == 'weibull':
                        save_dir = os.path.join(args.save_dir, args.model_dist + '_' + args.name + '_ds_' + args.dataset + '_lam' + str(args.lam) + '_bs' + str(args.batch_size) + '_lr' + str(args.lr) + '_optim' + str(args.optimizer) + '_epoch' + str(args.num_epochs) + '_seed' + str(args.seed))
                    else:
                        save_dir = os.path.join(args.save_dir, args.model_dist + '_' + args.name + '_ds_' + args.dataset + '_lam' + str(args.lam) + '_dr' + str(args.dropout_rate) + '_bs' + str(args.batch_size) + '_lr' + str(args.lr) + '_optim' + str(args.optimizer) + '_epoch' + str(args.num_epochs) + '_seed' + str(args.seed))
        except:
