...
class ModelEvaluator_test_km(object):
    ...
    def _eval_phase(self, model, data_loader, phase, device):
        ...
        for src, tgt in data_loader:
            ...
            if self.model_dist in ['cat', 'mtlr']:
                tgt2 = util.cat_bin_target(self.args, tgt, self.bin_boundaries) # check again
            ...
            if self.model_dist in ['cat', 'mtlr']:
                cdf = util.get_cdf_val(pred_params, tgt2, self.args)
                loss = loss_fn(pred_params, tgt2, model_dist=self.model_dist)
            ...
            if self.model_dist in ['cat', 'mtlr']:
                km_value = util.km_calibration2(pred_params, tgt, tgt2, self.args)

            else:
                km_value = util.km_calibration3(pred_params, tgt, self.args)
        ...
        if phase == 'test':
            ...
            sheet.cell(row=last_row+1, column=3, value=metrics[phase + '_' + 'scal(20)'].item())
            sheet.cell(row=last_row+1, column=4, value=metrics[phase + '_' + 'dcal(20)'].item())
            sheet.cell(row=last_row+1, column=5, value=metrics[phase + '_' + 'KS'].item())
            # sheet.cell(row=last_row+1, column=7, value=metrics[phase + '_' + 'KScal'].item())
            ...
            sheet.cell(row=last_row+1, column=8, value=(f'{self.args.model_dist}_{self.args.dataset}_km_lam_{self.args.lam}'))
